      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {products.slice(0, 8).map((product) => (
            <div
              key={product.id}
              className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-200 flex flex-col items-center p-4 animate-fade-in w-full"
            >
              <img
                src={product.image}
                alt={product.name}
                className="w-full max-w-[150px] h-40 object-cover rounded-md mb-4 transition-transform duration-300 hover:scale-105"
              />
